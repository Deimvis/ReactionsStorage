...
func (u *UserImpl) AddReaction() {
	...
	options := getAddReactionOptions(e)
	// TODO: remove
	u.logger.Debugw("Options to add reaction", "user", u.GetId(), "entity_id", e.GetId(), "options", options, "my_reactions", e.GetMyReactionIds())
	reactionId := options[rand.Intn(len(options))]
	u.app.AddReaction(e, u.GetId(), reactionId).Wait()
	// TODO: remove
	u.logger.Debugw("After add reaction", "user", u.GetId(), "entity_id", e.GetId(), "options", options, "my_reactions", e.GetMyReactionIds())
}
